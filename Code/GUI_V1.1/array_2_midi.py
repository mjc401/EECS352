...
def array_to_MIDI(array):
	...
	test_duration = (np.diff(np.sort(np.concatenate((array[:,1],array[:,2])))* 512./44100*1000)).astype(int)
	print array
	print test_duration
	#* 512./44100*960)
	# write MIDI file
	with MidiFile(ticks_per_beat = 500) as outfile:
		# Initialize
		track = MidiTrack()
		...
		for segment in xrange(0,array.shape[0]):
			if segment == 0:
				track.append(Message('note_on', note=array[0,0], velocity=array[0,3], time=int(array[0,1]*512./44100*1000)))
