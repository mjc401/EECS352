	...
	with MidiFile() as outfile:
		...
		for segment in xrange(0,array.shape[0]):
			if segment == 0:
				...
				track.append(Message('note_on', note=array[segment,0], velocity=array[segment,3], time=test_duration[seg_skip]))
				track.append(Message('note_off', note=array[segment,0], velocity=array[segment,3], time=test_duration[seg_skip + 1]))
				print test_duration[seg_skip + 1]
				seg_skip = segment + 2
